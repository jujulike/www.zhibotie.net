...
// | Update : 2012-04-19 
// +----------------------------------------------------------------------
jQuery.fn.ajax_city_select = function (city_config){
	//------------------基本函数----------------
				...
				if(result){			
					...
					if(_is_string(default_select)){
						for(var i= 0; i< jsondata.length; i++){
							...
							**/
							if(default_select == jsondata[i].name){						
								$('#'+select_id)[0].options.add(new Option(jsondata[i].name, jsondata[i].name,true,true));	
							}else{
								$('#'+select_id)[0].options.add(new Option(jsondata[i].name, jsondata[i].name));
							}
						}
					}else{
						for(var i= 0; i< jsondata.length; i++){
							$('#'+select_id)[0].options.add(new Option(jsondata[i].name, jsondata[i].name));	 //id				
						}
					}
				}
			}
