...
$feedlist = array();
if(ckprivacy('feed')) {
	$query = $_SGLOBAL['db']->query("SELECT f.*,((select count(*) from ".tname("docomment")." d where d.doid=f.id)) replynum FROM ".tname('feed')." f WHERE f.uid='$space[uid]' ORDER BY f.dateline DESC LIMIT 0,20");
	while ($value = $_SGLOBAL['db']->fetch_array($query)) {
...
$photos = $p_list = array();
if($space['albumnum'] && ckprivacy('album')){
	$query = $_SGLOBAL['db']->query("SELECT * FROM ".tname('pic')." WHERE uid='$space[uid]' ORDER BY dateline DESC LIMIT 0,9");
	...
	if($p_list){
		for($i = 0;$i<=8;$i++){
			//处理200X200;
			$photos[pic][] = pic_get($p_list[$i]['filepath'], $p_list[$i]['thumb'], $p_list[$i]['remote']);
			$photos['picid'][] = $p_list[$i]['picid'];
			$photos['hash'][] = md5($p_list[$i]['picid']);
			$photos['aid'][] = $p_list[$i]['albumid'];
		}
	}
