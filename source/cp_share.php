	...
	if(submitcheck('sharesubmit')) {
		...
		if($type == 'link') {
			...
			if(preg_match("/(youku.com|youtube.com|5show.com|ku6.com|sohu.com|mofile.com|sina.com.cn)$/i", $parseLink['host'], $hosts)) {
				...
				$flashimg = getflash_img($link,$hosts[1]);
                $flashtitle = getflash_title($link,$hosts[1]);
				if(!empty($flashvar)) {
					$arr['title_template'] = cplang('share_video')."：<a href=\"{$link}\" target=\"_blank\" title=\"{$flashtitle}\">{$flashtitle}</a>";
					$type = 'video';
					$arr['body_data']['flashvar'] = $flashvar;
					$arr['body_data']['host'] = $hosts[1];
					$arr['body_data']['flashimg'] = $flashimg;
                    $arr['body_data']['flashtitle'] = $flashtitle;
				}
			}
			// �ж��Ƿ����� mp3��wma
...
function getflash_img($link,$host){
	if('youku.com' == $host) {
		// http://v.youku.com/v_show/id_XNDg1MjA0ODg=.html
		preg_match_all("/id\_(\w+)[=.]/", $link, $matches);
		if(!empty($matches[1][0])) {
			$id = $matches[1][0];
			$result = getJsonByuri("http://v.youku.com/player/getPlayList/VideoIDS/{$id}");
			return $result['data']['0']['logo'];
		}
	}elseif('tudou.com' == $host) {
        // http://www.tudou.com/programs/view/oOrUqgezkOc/
		$tudouid = end(explode("/", $link));
		$matches = explode(".", $tudouid);
        $id = $matches[0];
        $result = getJsonByuri("http://api.tudou.com/v3/gw?method=item.info.get&appKey=092f8b4ac2b5a510&format=json&itemCodes={$id}");
        return $result['multiResult']['results']['0']['picUrl'];
	}else{
    	return "";
    }
}

function getflash_title($link,$host){
	if('youku.com' == $host) {
		// http://v.youku.com/v_show/id_XNDg1MjA0ODg=.html
		preg_match_all("/id\_(\w+)[=.]/", $link, $matches);
		if(!empty($matches[1][0])) {
        ...
        $id = $matches[0];
        $result = getJsonByuri("http://api.tudou.com/v3/gw?method=item.info.get&appKey=092f8b4ac2b5a510&format=json&itemCodes={$id}");
        return $result['multiResult']['results']['0']['title'];
	}else{
    	return "";
    }
}

function getJsonByuri($uri){
    $ch = curl_init();
