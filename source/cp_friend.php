...
if($op == "friends_add"){
    $array = explode(",",$_POST['uid']);
    foreach($array as $key => $value){
        if($value){
        	//判断是否是好友
        	$status = getfriendstatus($_SGLOBAL['supe_uid'],$value);
        	if($status == -1){
	            $setarr = array('uid' => $_SGLOBAL['supe_uid'],
							'fuid' => $value,
							'fusername' => addslashes($tospace['username']),
							'gid' => 1,
							'note' => getstr(cplang("add_u_friend"), 50, 1, 1),
							'dateline' => $_SGLOBAL['timestamp']
						);
	            inserttable('friend', $setarr);
	            //发送好友邮件请求
	            smail($value, '', cplang('friend_subject',array($_SN[$space['uid']], getsiteurl().'cp.php?ac=friend&amp;op=request')), '', 'friend_add');
	            //升级主表
	            $_SGLOBAL['db']->query("UPDATE ".tname('space')." SET addfriendnum=addfriendnum+1 WHERE uid='$value'");
        	}
        }
    }
    echo ajax_return(1,"ok");
    exit();
}elseif($op == 'del_friends'){
	$uid = $_POST['uid'];
	if($uid){
...
elseif($op == 'add') {
		...
		if($fstatus == -1) {
			...
			if(submitcheck('add2submit')) {
				...
				if(ckprivacy('friend', 1)) {
					...
					$fs['title_data'] = array('touser'=>"<a href=\"space.php?uid=$tospace[uid]\" class=\"bind_hover_card\" bm_user_id=\"$value[uid]\">".$_SN[$tospace['uid']]."</a>");
	...
	if(!empty($_GET['searchsubmit']) || !empty($_GET['searchmode'])) {
		...
		//����
		foreach (array('sex','qq','msn','birthyear','birthmonth','birthday','blood','marry','birthprovince','birthcity','birtharea','resideprovince','residecity','residearea') as $value) {
			if($_GET[$value]) {
