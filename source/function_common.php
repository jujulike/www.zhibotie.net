...
function inserttable($tablename, $insertsqlarr, $returnid=0, $replace = false, $silent=0) {
    ...
    $insertkeysql = build_implode(array_keys($insertsqlarr),1);
    $insertvaluesql = build_implode($insertsqlarr);
	 ...
	 * @return string
	 */
function build_insertmore($field, $data) {
		$field = ' (' . build_implode($field, 1) . ') '; //??????
		$temp_data = array();
		$data = (array) $data;
		foreach ($data as $val) {
			$temp_data[] = '(' . build_implode($val) . ')';
		}
	 ...
	 * @return string 
	 */
function build_implode($val, $iskey = 0) {
		if (!is_array($val) || empty($val)) return '';
		return implode(',', build_escape($val, $iskey));
	}
	 ...
	 * @return string
	 */
function build_escape($val, $iskey = 0) {
		if (is_array($val)) {
			foreach ($val as $k => $v) {
				$val[$k] = trim(build_escape_single($v, $iskey));
			}
			return $val;
		} 
		return build_escape_single($val, $iskey);
	}
...
/**
	 ...
	 * @return string
	 */
function build_escape_single($val, $iskey = 0) {
		if ($iskey === 0) {
			if (is_numeric($val)) {
				return " '" . $val . "' ";
			} else {
...
function ob_out() {
	...
	if($_SCONFIG['allowrewrite']) {
		...
		$preg_searchs[] = "/\<a href\=\"cp\.php\?ac\=([a-z0-9\=\&]+?)\"/ie";
		$preg_searchs[] = "/\<a href\=\"cp\.php\"/i";
		
		$preg_searchs[] = "/\<a href\=\"do\.php\?ac\=([a-z0-9\=\&]+?)\"/ie";
		$preg_searchs[] = "/\<a href\=\"do\.php\"/i";
		
		$preg_searchs[] = "/\<a href\=\"admincp\.php\"/i";
		$preg_searchs[] = "/\<a href\=\"help\.php\?ac\=([a-z0-9\=\&]+?)\"/ie";
		$preg_searchs[] = "/\<a href\=\"help\.php\"/i";
		
        $preg_searchs[] = "/\<a href\=\"live\.php\?do\=([a-z0-9\=\&]+?)\"/ie";
        $preg_searchs[] = "/\<a href\=\"live\.php\"/i";
		//加入index.php页面
        $preg_searchs[] = "/\<a href\=\"index\.php\"/i";
        $preg_searchs[] = "/\<a href\=\"taobao\.php\"/i";
        $preg_searchs[] = "/\<a href\=\"taobao\.php\?option\=([a-z0-9\=\&]+?)\"/ie";
        //加入feedrss.php
        $preg_searchs[] = "/\<a href\=\"feedRss\.php\"/i";
        
        
		$preg_replaces[] = 'rewrite_url(\'space-\',\'\\2\')';
		$preg_replaces[] = '<a href="'.geturl().'space.html"';
		
		$preg_replaces[] = 'rewrite_url(\'cp-\',\'\\1\')';
		$preg_replaces[] = '<a href="'.geturl().'cp.html"';
		
		$preg_replaces[] = 'rewrite_url(\'do-\',\'\\1\')';
		$preg_replaces[] = '<a href="'.geturl().'do.html"';
		
		$preg_replaces[] = '<a href="'.geturl().'admincp.html"';
		$preg_replaces[] = 'rewrite_url(\'help-\',\'\\1\')';
		$preg_replaces[] = '<a href="'.geturl().'help.html"';

        $preg_replaces[] = 'rewrite_url(\'live-\',\'\\1\')';
		$preg_replaces[] = '<a href="'.geturl().'live.html"';
		
		$preg_replaces[] = '<a href="'.geturl().'index.html"';
		$preg_replaces[] = '<a href="'.geturl().'taobao.html"';
		$preg_replaces[] = 'rewrite_url(\'taobao-\',\'\\1\')';
		$preg_replaces[] = '<a href="'.geturl().'rss.html"';
	...
	if($_SCONFIG['allowrewrite']) {
    	$content = preg_replace(
    			array(
    			      "/\<a href\=\"live\.php\?do\=(douban|tianya|baidu)\&key\=(.*?)\&searchkey=\&page=(.*?)\"/i",
    				  "/\<a href\=\"live\.php\?do\=(douban|tianya|baidu)\&key\=(.*?)\"/i",
    				  "/\<a href\=\"live\.php\?tags\=(.*?)\&page\=(.*?)\"/i",
    				  "/\<a href\=\"live\.php\?tags\=(.*?)\"/i"
    			),
    			array(
    			      "<a href=\"".geturl()."\\1/\\2-\\3.html\"",
    				  "<a href=\"".geturl()."\\1/\\2.html\"",
    				  "<a href=\"".geturl()."live-tags-\\1-\\2.html\"",
    				  "<a href=\"".geturl()."live-tags-\\1.html\""
    			), $content);
	}
	if($preg_searchs) {
	...
	if($_SCONFIG['allowrewrite']) {
	    $content = preg_replace("/\<a href\=\"live\.php\?do=view\&tid\=(.+?)\&page=(.+?)#(.*?)\"/i","<a href=\"".geturl()."live-view-tid-\\1-page-\\2.html#\\3\"",$content);
	    $content = preg_replace_callback("/\<a href\=\"live\.php\?do=view\&tid\=(.+?)#(.*?)\"/i","rewrite_back",$content);
	}
	obclean();
	if($_SGLOBAL['inajax']) {
		xml_out($content);
		...
		$lock = $memcache->get("{$_SGLOBAL['threadcache']['tid']}_{$_SGLOBAL['threadcache']['page']}_lock");
		$o = ceil( $_SGLOBAL['threadcache']['count']/100 );
		if($_SCONFIG['pagecache'] &&  $o > 1 && $_SGLOBAL['threadcache']['page'] < $o && !$lock && $_SGLOBAL['threadcache']['tid']){
			//生成lock 
			$memcache->save("{$_SGLOBAL['threadcache']['tid']}_{$_SGLOBAL['threadcache']['page']}_lock",1);
			swritefile($_SGLOBAL['threadcache']['filename'],$content);
			//删除
			$memcache->delete("{$_SGLOBAL['threadcache']['tid']}_{$_SGLOBAL['threadcache']['page']}_lock");
		}
		//释放
		unset($memcache);
		echo $content;
...
function rewrite_url($pre, $para) {
	...
	return '<a href="'.geturl().''.$pre.$para.'.html"';
}

function rewrite_back($match){
    return "<a href=\"".geturl()."live-view-tid-{$match[1]}.html#{$match[2]}\"";
}

//????
function iframe_url($url) {
	$url = rawurlencode($url);
	return "<a href=\"".geturl()."link.php?url=http://$url\"";
}
...
function debuginfo() {
	...
	if(empty($_SCONFIG['debuginfo'])) {
		...
		$info .= ($_SCONFIG['pagememcache'] ? ' , Memcache On.': NULL);
	}
...
function pic_get($filepath, $thumb, $remote, $return_thumb=1) {
	...
	if(empty($filepath)) {
		...
		$url = $filepath;
		//内裤叔叔  2012.08.01 修改此项 用于又拍云存储
		if($return_thumb && $thumb && $remote){
			$url .= "!small";//'.thumb.jpg';
		}
		if($remote) {
			...
			if($return_thumb && $thumb){
				$url = $_SC['attachurl'].$url.'.thumb.jpg';
			}else{
...
function pic_cover_get($pic, $picflag) {
	...
	if(empty($pic)) {
		...
		if($picflag == 1) {//????
		    ...
		    $url = $_SCONFIG['allowrewrite']? $pic."!small":$pic;
		} else {//????
			$url = $pic;
		}
	}
...
function mkfeed($feed, $actors=array()) {
	...
	$searchs[] = '{actor}';
    if($feed['uid']){
        $uri = "<a href=\"space.php?uid=$feed[uid]\" class=\"bind_hover_card\" bm_user_id=\"$feed[uid]\">".$feed['username']."</a>";
    }else{
        $uri = "<a href=\"javascript:;\">".$feed['username']."</a>";
    }
	$replaces[] = empty($actors)?$uri:implode(lang('dot'), $actors);
	$searchs[] = '{app}';
    ...
    if(strexists($feed['image_1'],".thumb.jpg")){
        $feed['image_1'] = str_replace(".thumb.jpg","",$feed['image_1']);
    }
    if(strexists($feed['image_2'],".thumb.jpg")){
        $feed['image_2'] = str_replace(".thumb.jpg","",$feed['image_2']);
    }
    if(strexists($feed['image_3'],".thumb.jpg")){
        $feed['image_3'] = str_replace(".thumb.jpg","",$feed['image_3']);
    }
    if(strexists($feed['image_4'],".thumb.jpg")){
        $feed['image_4'] = str_replace(".thumb.jpg","",$feed['image_4']);
    }
...
function avatar_path($uid){
	...
	$dir2 = substr($uid, 3, 2);
	$dir3 = substr($uid, 5, 2);
	$data['file'] = $dir1.'/'.$dir2.'/'.$dir3.'/'.substr($uid, -2);
	$data['path'] = $dir1.'/'.$dir2.'/'.$dir3.'/';
	return $data;
}
...
function qqconnect() {
     ...
     * @param 楂樹寒??楃涓?
     * @author 鍐呰￥鍙斿彅
     * @param $string string 鍘熷瓧绗︿?D锛?
     * @param $keyword string 鏌ユ壘??楃涓诧??
     * @param $color string 楂樹寒棰滆壖 榛樿涓猴細#FFFF00锛?
     * @return string;
     */
function find_string($string,$keyword,$color = ""){
      if(!$color) $color = "#FFFF00";
      if($string && $keyword){
            return preg_replace("/$keyword/","<span style=\"background:$color;\">$keyword</span>",$string);
        }
    }

/**
	 *	鎺у埗鍣? AJAX鑴氭湰杈撳嚭
	 *  Controller涓娇鐢ㄦ柟娉曪??this->controller->ajax_return()
	 * 	@param  int     $status  0:閿欒淇℃伅|1:姝ｇ‘淇℃伅
	 * 	@param  string  $message  鏄剧ず鐨勪俊鎭?
	 * 	@param  array   $data    浼犺緭鐨勪俊鎭?
	 * 	@param  array   $type    杩斿洖鏁版嵁绫诲瀷锛宩son|xml|eval
	 *  @return object
	 */
    ...
    */
    function SafeHtml($msg = "")
    {
     ...
     return $msg;
        ...
        /**
		 * 缁欐暟缁勬帓閲?
		 * 涓巃rray_unique鍑芥暟鐨勫尯鍒細??冭姹??al鏄瓧绗︿?D锛岃?岃繖涓彲浠ユ槸鏁扮粍/瀵硅??
		 *
		 * @param array $arr 瑕佹帓閲嶇殑鏁扮??
		 * @param bol $reserveKey 鏄惁淇濈暀鍘熸潵鐨凨ey
		 * @return unknown
		 */
...
function del_file($filename) {
   ...
   /*
    *@param 获取地址所在内容
    *@param $url string 地址，只允许http://形式存在
    *@param $module 模式，允许 0.1.2等形式存在
    *               0 默认 ， file_get_contents 函数获取，缺点有
    *               1 php curl 模式获取 ，可伪造 header、cookies等信息；
    *@param $header array(); 
    *				仅仅在 $module == 1 的情况下有效
    *				为数组如：array("Referer: http://www.baidu.com/")
    *				具体浏览器http报头可用 @get_headers 进行查看；
    */
function fopen_url($url,$module = 1,$header = array()) 
	{ 
		switch ($module){
			case 0:
				$file_content = (function_exists("file_get_contents"))?@file_get_contents($url):"";
				break;
			case 1:
				$ch = curl_init();
				...
				ob_start();
				curl_exec($ch);
				$file_content['content'] = ob_get_contents();
				ob_end_clean();
				$file_content['header'] = curl_getinfo($ch);
				curl_close($ch);
				break;
			case 2:
				break;
     ...
     * @param $lenth int
     * @return array();
     * @example join("",subString_UTF8($str, $start, $lenth))
     */
    ...
    * @return string;
    */
    function cubstr_same($string){
    	return preg_replace("/(！|？|—|~|==|。|--|@|#|!)\\1+/iu", "$1", $string);
    	//return preg_replace("/([\x{4e00}-\x{9fa5}]|.)\\1+/iu", "$1", $string);
    }
	...
	/**
		...
		@return array();
	*/
	function short_url_163($uri){
		$url = "http://126.am/api!shorten.action";
		$post_array = "key=0ef9301711244276ab546a21f1e426eb&longUrl=".urlencode($uri);
		$ch = curl_init();  
 ...
 */
     ...
     * @param int $tid;
     */
    function inserttag($data = array(),$tid){
        global $_SGLOBAL;
    	if(is_array($data) && $data['status'] == "ok" && $tid){
    		$array  = array("`","!","@","#","$","%","^","&","*","(",")","-","+","=","[","{","}","]","\\","|","<",">",",",".","?","/","\"","'",":",";","“","”","‘","’","：","；","，","。","？","！");
            foreach ($data['words'] as $key => $value){
            	//替换标点符号
            	$value['word'] = str_replace($array, "", $value['word']);
            	if($value[word] && strlen($value['word'])>3 ){
            		$taglist .= $value['word'].",";
            		$sql = "SELECT * FROM ".tname("threads_tag")." WHERE tagname='{$value['word']}'";
                    $res = $_SGLOBAL['db']->fetch_array($_SGLOBAL['db']->query($sql));
                    if($res){
                        updatetable("threads_tag",array("count"=>$res['count'] + 1,"dateline"=>time()),array("tagid"=>$res['tagid']));
                        $tagid = $res['tagid'];
                    }else{
                        $tagid = inserttable("threads_tag",array("tagname"=>$value['word'],"ishot"=>0,"count"=>1,"dateline"=>time()),1);
                    }
                    inserttable("threads_tag_filed",array("tagid"=>$tagid,"tid"=>$tid));
            	}
            }
            updatetable("threads",array("taglist"=>$taglist),array("tid"=>$tid));
    		return true;
    	}else{
    		return false;
    	}
     ...
     * @return string
     */
    function replace_ubb_html($string){
    	//先处理空格和换行符
    	$string = str_replace(array("\r\n","\r","\n"," ","　"), "",cubstr_same(strip_tags($string)));
    	//处理 ubb代码
    	$replace = array("/\[url(.*?)url|tag\=(.*?)\](.*?)\[\/url\]/is","/\[img\](.*?)\[\/img\]/is","/\[color\=(.*?)\](.*?)\[\/color\]/is");
    	$replace_re = array("","","\\2 ");
    	$string = preg_replace($replace,$replace_re, $string);
     ...
     * @return array
     */
    function get_friend_tree($uid,$head = false,$limit = 20){
    	global $_SGLOBAL;
    	if(!$uid) return false;
