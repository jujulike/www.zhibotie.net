...
class IpLocation
{
    ...
    public function getstring($data = '')
    {
        ...
        return ($this->isgbk)?$data:iconv("gbk","utf-8",$data);;
    }
    /**
     * 返回地区信息
     * @access private
     ...
     * @return IpLocation
     */
    public function IpLocation($filename = '',$isgbk = true)
    {
    	$this->isgbk=$isgbk;
        if (!$filename) {
            $filename = dirname(__FILE__) . '/qqwry/QQWry.Dat';
        }
        if (!file_exists($filename)) {
            die('qqwry.dat is not exists!');
        }
