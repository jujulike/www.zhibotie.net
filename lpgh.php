...
$uri = "http://www.lerhuo.com/a/class/%E5%A4%A9%E6%B6%AF%E7%A4%BE%E5%8C%BA/";

$filename = "./data/getLZ.txt";
...
if(!$content){
	...
	preg_match_all("/<li class=\"typost\">(.*?)<\/li>/is", $content['content'], $daitl);
	foreach ($daitl[1] as $key => $value){
		preg_match("/href=\"http:\/\/www\.lerhuo\.com\/postcache\/(.*?)\/page\.html/is", $value , $uri);
		$url = "http://www.lerhuo.com/postcache/{$uri[1]}/page.html";
		$newcontent = fopen_url($url);
		preg_match("/<li>分类：(.*?)<\/li>/is", $newcontent['content'], $newuri);
			...
			if($return){
				...
				foreach($return["reply"]["message"] as $key => $v){
					if($return['hash'] ==  $return["reply"]["hash"][$key]){
						...
						$posts['subject'] = mysql_real_escape_string($return['subject']);
						$posts['message'] = html2bbcode($v);
						$posts['author'] = mysql_real_escape_string($return["reply"]["author"][$key]);
						$posts['fid'] = $fid;
						$posts['tid'] = $tid;
						$posts['frist'] = 0;
						$posts['head'] = "";
				...
				if($tid && $pid){
					...
					echo "next";
					echo "<meta http-equiv=\"refresh\" content=\"1;URL=?key={$nextkey}\" />";
					exit();
				}else{
					echo "next";
					echo "<meta http-equiv=\"refresh\" content=\"1;URL=?key={$nextkey}\" />";
